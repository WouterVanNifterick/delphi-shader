...
begin
	...
	dist  := sphere(ro, rd,
				  Vec3.Create(system.sin(t*3)*3,system.cos(t*3)*3,system.cos(t)*8),
				  1.5, vec3_2,
				  nml, mat, dist);
	dist  := sphere(ro, rd,
				  Vec3.Create(system.sin(t*3)*-3,system.cos(t*3)*-3,system.sin(t)*8),
				  1.5, vec3_3,
				  nml, mat, dist);
	Exit( dist );
end;
...
begin
	...
	Exit( pow(math.max(0, dot(sunDir, rd)), 128)*sunColor + 0.2*pow(math.max(0, dot(sunDir, rd)), 2)*sunColor + pow(math.max(0, -dot(vec3_7, rd)), 1)*(1-skyColor) + pow(math.max(0, dot(vec3_6, rd)), 1)*skyColor );
end;
