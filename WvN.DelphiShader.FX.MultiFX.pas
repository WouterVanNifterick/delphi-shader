...
//	position  := rotated_position;
 ...
 if effect_number=10 then
	begin
		...
		begin
			...
			gradient  := gradient  + (((fade / 384) / pow(length(centered_coord - blob_coord), 1.5)) * (fade * fade));
		end;
 ...
 if effect_number=11 then
	begin
		z  := system.sqrt(0.25 - centered_coord.x * centered_coord.x - centered_coord.y * centered_coord.y);
 ...
 if effect_number=15 then
	begin
		gradient  := system.sin(coord.x * 32 + 2 * time) +
                 system.sin(coord.x * 16 + coord.y * 24) +
                 system.sin(coord.x * 4 + system.sin(coord.y * 18 + 4*time)) +
                 system.sin(system.sin((coord.x + coord.y) * 33) + system.sin(coord.y * 24 + time));
		color  := Vec3.Create(system.sin(gradient * 2) * 0.5 + 0.5,
                          system.sin(gradient * 1.5) * 0.75 + 0.25,
                          system.sin(gradient * 1.2) * 0.5 + 0.5);
	end;
...
// =================
	...
	begin
		...
		sum  := sum  + texture2D(backbuffer, tex_coord + displacement) * light_power;
	end;
