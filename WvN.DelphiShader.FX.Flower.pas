...
begin
  ...
  a := atan(p.x, p.y);
  r := length(p) * 0.75;
  w := cosLarge(3.1415927 * time - r * 2.0);

  h := 0.5 + 0.5 * system.cos(12.0 * a - w * 7 + r * 8);
  ...
  col    := col * (1.25 + 0.25 * system.cos((12 * a - w * 7 + r * 8) / 2));
  col    := col * (1 - 0.35 * (0.5 + 0.5 * system.sin(r * 30)) * (0.5 + 0.5 * system.cos(12 * a - w * 7 + r * 8)));
  Result := TColor32(vec3.Create(col, col - h * 0.5 + r * 0.2 + 0.35 * h * (1 - r), col - h * r + 0.1 * h * (1 - r)));
end;
